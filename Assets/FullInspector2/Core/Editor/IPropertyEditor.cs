...
namespace FullInspector {
    ...
    /// </para>
    /// <para>
    /// You almost certainly want to extend PropertyEditor{T} instead of this interface. It
    /// provides an automatic implementation for all methods and gives type-safety to your editor.
    /// </para>
    /// </remarks>
    ...
    public abstract class PropertyEditor<TElement> : IPropertyEditor, IPropertyEditorEditAPI {
        ...
        object IPropertyEditorEditAPI.Edit(Rect region, GUIContent label, object element) {
            ...
            return Edit(region, label, (TElement)element);
        }
        float IPropertyEditorEditAPI.GetElementHeight(GUIContent label, object element) {
            if (element != null && element is TElement == false) {
                Debug.LogError("Property editor " + GetType().FullName +
                    " cannot be used on an object of type " + element.GetType());
                return 0;
            ...
            return GetElementHeight(label, (TElement)element);
        }
        ...
        public virtual TElement Edit(Rect region, GUIContent label, TElement element) {
            var nextEditor = EditorChain.GetNextEditor(this);
            return PropertyEditorExtensions.Edit(nextEditor, region, label, element);
        }
        public virtual float GetElementHeight(GUIContent label, TElement element) {
            var nextEditor = EditorChain.GetNextEditor(this);
            return PropertyEditorExtensions.GetElementHeight(nextEditor, label, element);
        }
