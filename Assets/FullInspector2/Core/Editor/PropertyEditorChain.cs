...
namespace FullInspector {
    ...
    public class PropertyEditorChain {
        ...
        /// <summary>
        /// Attempts to identify an infinite recursion cycle between the two editing chains. If a
        /// cycle is detected, then a property editor can take intelligent action to stop the
        /// infinite recursion.
        /// </summary>
        public bool HasCycle(PropertyEditorChain other) {
            // two chains are in a cycle if they each follow the same path

            if (_editors.Count != other._editors.Count) {
                return false;
            }

            for (int i = 0; i < _editors.Count; ++i) {
                if (_editors[i] != other._editors[i]) {
                    return false;
                }
            }
