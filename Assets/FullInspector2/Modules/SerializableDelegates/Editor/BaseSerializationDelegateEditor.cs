...
namespace FullInspector.Modules.SerializableDelegates {
    public abstract class BaseSerialiationInvokableEditor<TSerializationDelegate> : PropertyEditor<TSerializationDelegate>
        ...
        /// <summary>
        /// Returns the methods that should be shown in the dropdown, and returns the active method in that list.
        /// </summary>
        private void GetMethodOptions(TSerializationDelegate action, out int selectedIndex, out string[] displayedOptions) {
            ...
            for (int i = 0; i < containerType.Methods.Length; ++i) {
                var method = containerType.Methods[i];
        ...
        public override TSerializationDelegate Edit(Rect region, GUIContent label, TSerializationDelegate element) {
        ...
        public override float GetElementHeight(GUIContent label, TSerializationDelegate element) {
            EnsureInstance(ref element);
